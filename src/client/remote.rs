...
use crate::client::rpc::{
    TSCreateMultiTimeseriesReq, TSCreateTimeseriesReq, IClientRPCServiceSyncClient, TSInsertRecordsReq,
    TSInsertStringRecordReq, TSInsertTabletReq, TSOpenSessionReq, TSProtocolVersion,
    TIClientRPCServiceSyncClient,
};
use crate::protocal::{
...
pub struct Config {
    ...
    #[builder(default = TSProtocolVersion::IotdbServiceProtocolV3)]
...
pub struct RpcSession {
    ...
    statement_id: i64,
    client: IClientRPCServiceSyncClient<Box<dyn TInputProtocol>, Box<dyn TOutputProtocol>>,
}
...
impl RpcSession {
    pub fn new(config: Config) -> Result<Self> {
        ...
        Ok(Self {
            ...
            session_id: None,
            statement_id: -1,
            client: IClientRPCServiceSyncClient::new(input_protocol, output_protocol),
        })
    }
}
...
impl<'a> Session<'a> for RpcSession {
    fn open(&mut self) -> Result<()> {
        ...
        let result = self.client.request_statement_id(resp.session_id.unwrap());
        match result {
            Ok(stId) => {
                self.statement_id = stId;
            }
            Err(_) => {}
        }
        ...
        if let Some(session_id) = self.session_id {
            let resp = self
                ...
                .execute_raw_data_query(super::rpc::TSRawDataQueryReq {
                    ...
                    enable_redirect_query: None,
                    jdbc_query: None,
                    timeout: Option::from(timeout_ms),
                    legal_path_nodes: Option::from(false)
                })?;
            let status = resp.status;
            let code = status.code;
            if code == SUCCESS_STATUS {
